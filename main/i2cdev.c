...
#define SEMAPHORE_TAKE(port)                                                          \
    ...
    {                                                                                 \
        ...
        {                                                                             \
            ESP_LOGE(I2C_TAG, "Could not take port mutex %d", port);                      \
...
#define SEMAPHORE_GIVE(port)                                     \
    ...
    {                                                            \
        ...
        {                                                        \
            ESP_LOGE(I2C_TAG, "Could not give port mutex %d", port); \
            return ESP_FAIL;                                     \
        }                                                        \
    } while (0)
//#endif
...
esp_err_t i2cdev_init() {
    ...
    for (int i = 0; i < I2C_NUM_MAX; i++) {
        ...
        if (!states[i].lock) {
            ESP_LOGE(I2C_TAG, "Could not create port mutex %d", i);
            return ESP_FAIL;
        }
    }
#endif
...
esp_err_t i2c_dev_create_mutex(i2c_dev_t *dev) {
    ...
    ESP_LOGV(I2C_TAG, "[0x%02x at %d] creating mutex", dev->addr, dev->port);
...
esp_err_t i2c_dev_take_mutex(i2c_dev_t *dev) {
    ...
    ESP_LOGV(I2C_TAG, "[0x%02x at %d] taking mutex", dev->addr, dev->port);
...
esp_err_t i2c_dev_give_mutex(i2c_dev_t *dev) {
    ...
    ESP_LOGV(I2C_TAG, "[0x%02x at %d] giving mutex", dev->addr, dev->port);
...
cfg_equal(const i2c_config_t *a, const i2c_config_t *b) {
...
static esp_err_t i2c_setup_port(const i2c_dev_t *dev) {
    ...
    if (!cfg_equal(&dev->cfg, &states[dev->port].config) || !states[dev->port].installed) {
        ESP_LOGD(I2C_TAG, "Reconfiguring I2C driver on port %d", dev->port);
        i2c_config_t temp;
        ...
        ESP_LOGD(I2C_TAG, "I2C driver successfully reconfigured on port %d", dev->port);
    }
#if HELPER_TARGET_IS_ESP32
    int t;
    ...
    if ((ticks != t) && (res = i2c_set_timeout(dev->port, ticks)) != ESP_OK)
        return res;
    ESP_LOGD(I2C_TAG, "Timeout: ticks = %" PRIu32 " (%" PRIu32 " usec) on port %d", dev->timeout_ticks, dev->timeout_ticks / 80, dev->port);
#endif
...
esp_err_t i2c_dev_read(const i2c_dev_t *dev, const void *out_data, size_t out_size, void *in_data, size_t in_size) {
    ...
    if (res == ESP_OK) {
        ...
        res = i2c_master_cmd_begin(dev->port, cmd, pdMS_TO_TICKS(1000));
        if (res != ESP_OK)
            ESP_LOGE(I2C_TAG, "Could not read from device [0x%02x at %d]: %d (%s)", dev->addr, dev->port, res,
                     esp_err_to_name(res));
...
i2c_dev_write(const i2c_dev_t *dev, const void *out_reg, size_t out_reg_size, const void *out_data, size_t out_size) {
    ...
    if (res == ESP_OK) {
        ...
        res = i2c_master_cmd_begin(dev->port, cmd, pdMS_TO_TICKS(1000));
        if (res != ESP_OK)
            ESP_LOGE(I2C_TAG, "Could not write to device [0x%02x at %d]: %d (%s)", dev->addr, dev->port, res,
                     esp_err_to_name(res));
        i2c_cmd_link_delete(cmd);
    }
