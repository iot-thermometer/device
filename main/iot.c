...
static const char remote_device_name[] = "realme GT Master Edition";
#define REMOTE_SERVICE_UUID 0x1805
#define REMOTE_NOTIFY_CHAR_UUID 0x2A2b
...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_READ_CHAR_EVT:
        if (param->read.status == ESP_GATT_OK)
        {
            ...
            {
                printf("%d.%d %d:%d:%d\n", p_data->read.value[3], p_data->read.value[2], p_data->read.value[4], p_data->read.value[5], p_data->read.value[6]);
                sleep(1);
